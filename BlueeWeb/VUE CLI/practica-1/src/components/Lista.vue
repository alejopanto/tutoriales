<template>
    <div>
        <ul class="list-group">
            <li v-for="(fruta, index) in arrayOrdenado" :key="index" @click="aumentar(index)"
            class="list-group-item d-flex justify-content-between align-items-center">
                {{index}} - {{fruta.nombre}}
                <span class="badge badge-primary badge-pill">{{fruta.cantidad}}</span>
            </li>
        </ul>
        <button class="btn btn-warning btn-block mt-5" @click="reiniciar()">Reinicar Contadores</button>
    </div>
</template>
